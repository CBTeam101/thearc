<script type="text/javascript">
  const createModule = {
    ...
    cache: function() {
      ...
      this.recipient = this.container.find('[name=create-transfer-token-user-account]')
      this.tokentype = this.container.find('[name=create-transfer-token-token-type]')
      this.description = this.container.find('[name=create-transfer-token-description]')
      this.transferForm = this.container.find('#transfer-token-create-form')
      this.modal = this.container.find('#transfer-token-create-modal')
    },
    ...
    clear: function() {},
    events: function() {
      this.transferForm.on('submit', function(e) {
        e.preventDefault()
        const isValid = this.validate(e.currentTarget, e)
        if(!isValid) return
        const options = {
          url: '/operations/transfer-tokens',
          method: 'POST',
          data: {
