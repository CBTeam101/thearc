<script type="text/javascript">
  const createModule = {
    ...
    plugin: function() {
      ...
      this.paymentMethod.select2({
        ajax: {
        url: '/select2/payment-methods',
          data: function (params) {
            var query = {
              search: params.term
            }
    ...
    cache: function() {
      ...
      this.ask = this.container.find('[name=create-ask_price]')
      this.currentPrice =this.container.find('[name=create-current_price]')
      this.token = this.container.find('[name=create-token]')
      this.recipient = this.container.find('[name=create-sell-token-user-account]')
      this.tokentype = this.container.find('[name=create-sell-token-token-type]')
      this.paymentMethod = this.container.find('[name=create-sell-token-payment-method]')
      this.description = this.container.find('[name=create-sell-token-description]')
      this.sellForm = this.container.find('#sell-token-create-form')
      this.modal = this.container.find('#sell-token-create-modal')
    },
    ...
    clear: function() {},
    events: function() {
      this.sellForm.on('submit', function(e) {
        e.preventDefault()
        const isValid = this.validate(e.currentTarget, e)
        if(!isValid) return
        const options = {
          url: '/operations/sell-tokens',
          method: 'POST',
          data: {
